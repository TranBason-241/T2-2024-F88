...
const sendPostRequest = async () => {
  ...
  function chuyenDoiKhongDau(string) {
    chuoi = string.toLowerCase();
    var coDau = "àáảãạăắằẳẵặâấầẩẫậèéẻẽẹêếềểễệìíỉĩịòóỏõọôốồổỗộơớờởỡợùúủũụưứừửữựỳýỷỹỵđ";
    var khongDau = "aaaaaaaaaaaaaaaaaeeeeeeeeeeeiiiiiooooooooooooooooouuuuuuuuuuuyyyyyd";
    for (var i = 0; i < coDau.length; i++) {
      chuoi = chuoi.replace(new RegExp(coDau.charAt(i), "g"), khongDau.charAt(i));
    }
  ...
  phone = data2["Số điện thoại"];

  VaiTroLamChu = data2["2. Theo bạn, thông thường ai sẽ nắm vai trò “làm chủ” gia đình?"];
  targetDataVaiTroLamChu = chuyenDoiKhongDau(VaiTroLamChu);
  console.log(targetDataVaiTroLamChu);

  KenhVayTienCuaPhuNu = data2["7. Nếu cần tài chính để kinh doanh, phụ nữ sẽ tìm những kênh nào?"];
  targetDataKenhVayTienCuaPhuNu = chuyenDoiKhongDau(KenhVayTienCuaPhuNu);
  console.log(targetDataKenhVayTienCuaPhuNu);

  TinhTrangHonNhan = data2["10. Tình trạng hôn nhân của bạn"];
  targetDataTinhTrangHonNhan = chuyenDoiKhongDau(TinhTrangHonNhan);
  console.log(targetDataTinhTrangHonNhan);

  age_range = data2["9. Độ tuổi của bạn"];
  targetDataage_range = chuyenDoiKhongDau(age_range);
  console.log(targetDataage_range);

  dataCheckingFormBanner = {
    campaign_name: "survey_24-phu_nu_va_vai_tro_lam_chu", //name của ca
    ...
    index_topic: "",
    evt_info: {
      VaiTroLamChu: targetDataVaiTroLamChu,
      KenhVayTienCuaPhuNu: targetDataKenhVayTienCuaPhuNu,
      TinhTrangHonNhan: targetDataTinhTrangHonNhan,
      age_range: targetDataage_range,
      email: email,
      phone: phone,
      name: "",
    },
  ...
  api = "https://script.google.com/macros/s/AKfycbw4k9ouSRj2rbpvma86wK72fU6u0Zw9qtt5g1SaNaaAhNPbJVXxsYF69eF20ZaV5Ruzbw/exec";
  ...
  axios({
    ...
    .then(function (response) {
      ...
      if (response.status === 201 || response.status === 200) {
        ...
        dataLayer.push({
          event: "voteEvent",
          vote_category: "survey_24-phu_nu_va_vai_tro_lam_chu",
          vote_action: "survey_24-phu_nu_va_vai_tro_lam_chu",
        });
...
customQuestionNav = document.querySelectorAll("#q-1 input");
customQuestionNav.forEach((element) => {
